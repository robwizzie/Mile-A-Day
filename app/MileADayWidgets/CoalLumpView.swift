...
// (app/Mile A Day/Views/Components/ and app/MileADayWidgets/): the flame
// widget draws the hero's own coal when there's no streak. Edit one, `cp`,
// `cmp`. Depends only on `FlamePalette.lerp`, which both targets carry.
...
struct CoalLumpView: View {
    ...
    var warmth: Double = 0
    /// The finished frame with no clock (a widget renders statically).
