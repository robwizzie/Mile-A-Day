...
class AppleSignInManager: NSObject, ObservableObject {
    ...
    private func authenticateWithBackend(_ profile: AppleUserProfile, _ credential: ASAuthorizationAppleIDCredential) async throws -> BackendAuthResponse {
        ...
        do {
            ...
            throw AppleSignInError.backendError("Cannot connect to backend server: \(error.localizedDescription)")
        }
        ...
        let authData = [
            ...
            "email": profile.email ?? "",
            "full_name": profile.fullName?.formatted() ?? ""
        ]
        
        guard let url = URL(string: "\(backendURL)/auth/apple/authenticate") else {
            throw AppleSignInError.invalidURL
        }
