...
// MARK: - Active Competitions Card

struct ActiveCompetitionsCard: View {
    @ObservedObject var competitionService: CompetitionService
    @Environment(\.colorScheme) var colorScheme

    private var activeCompetitions: [Competition] {
        competitionService.competitions.filter { $0.status == .active }
    }
    ...
    var body: some View {
        if !activeCompetitions.isEmpty {
            VStack(alignment: .leading, spacing: 12) {
                ...
                // Competition mini-cards
                VStack(spacing: 8) {
                    ForEach(activeCompetitions.prefix(3)) { comp in
                        NavigationLink(destination: CompetitionDetailView(competition: comp, competitionService: competitionService)) {
                            ActiveCompetitionMiniRow(competition: comp)
                        }
                        .buttonStyle(PlainButtonStyle())
                    }
                }
                ...
                // Show more if > 3
                if activeCompetitions.count > 3 {
                    HStack {
                        Spacer()
                        Text("+\(activeCompetitions.count - 3) more")
                            .font(.system(size: 11, weight: .medium, design: .rounded))
                            .foregroundColor(.secondary)
                        Spacer()
                    }
            ...
            .task {
                if competitionService.competitions.isEmpty {
                    try? await competitionService.loadCompetitions()
                }
            }
        }
    }
...
// MARK: - Active Competition Mini Row

struct ActiveCompetitionMiniRow: View {
    ...
    private var currentUser: CompetitionUser? {
        competition.users.first(where: { $0.user_id == currentUserId })
    }
        ...
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                HStack(spacing: 6) {
                    ...
                    Text("\(competition.acceptedUsersCount) people")
                        .font(.system(size: 10, weight: .medium, design: .rounded))
                        .foregroundColor(.secondary)
                }
            }
    ...
    private var scoreView: some View {
        if let user = currentUser {
            ...
            case .race:
                let progress = min(score / max(competition.options.goal, 0.1), 1.0)
