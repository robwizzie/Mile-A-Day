...
async function fetchStreaks(userIds: string[]): Promise<Record<string, number>> {
	if (userIds.length === 0) return {};
	try {
		const rows = await db.query(`SELECT user_id, current_streak FROM users WHERE user_id = ANY($1::text[])`, [userIds]);
		...
		console.error('fetchStreaks failed (likely pre-leaderboard schema):', err.message);
		return {};
	}
}
