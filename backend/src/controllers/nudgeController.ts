...
export async function nudgeUser(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    ...
    const senderInComp = competition.users.find(
      (u) => u.user_id === senderId && u.invite_status === "accepted",
    );
    const targetInComp = competition.users.find(
      (u) => u.user_id === targetUserId && u.invite_status === "accepted",
    );

    if (!senderInComp) {
      return res
        .status(403)
        .json({ error: "You are not a participant in this competition" });
    }
    if (!targetInComp) {
      return res.status(400).json({
        error: "Target user is not a participant in this competition",
      });
    }
    ...
    if (!allowed) {
      return res
        .status(429)
        .json({ error: "You can only nudge this person once every 24 hours" });
    }
