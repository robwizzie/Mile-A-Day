...
export async function updateUser(req: Request, res: Response) {
  ...
  if (!updates.length) {
    return res
      .status(400)
      .json({ error: "No valid update fields present in request." });
  }
...
export async function deleteUser(req: Request, res: Response) {
  ...
  const workoutIds = await db.query(
    "SELECT workout_id FROM workouts WHERE user_id = $1",
    [userId],
  );
  if (workoutIds.length) {
    const ids = workoutIds.map((w: any) => w.workout_id);
    await db.query(`DELETE FROM workout_splits WHERE workout_id = ANY($1)`, [
      ids,
    ]);
  }
  ...
  try {
    ...
    if (existingUser.length && existingUser[0].profile_image_url) {
      const oldPath = path.join(
        process.cwd(),
        existingUser[0].profile_image_url,
      );
