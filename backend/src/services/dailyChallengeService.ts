...
export async function getTodaysChallenge(userId: string, localDate: string): Promise<TodaysChallengeResponse> {
	...
	const progress = completionRow ? 1.0 : await computeProgress(userId, localDate, challengeRow.challenge_key, goalMiles);
...
async function computeProgress(userId: string, localDate: string, challengeKey: string, goalMiles: number): Promise<number> {
	switch (challengeKey) {
		case 'double_down': {
...
async function evaluatePredicate(userId: string, localDate: string, challengeKey: string, goalMiles: number): Promise<boolean> {
	switch (challengeKey) {
		case 'double_down':
...
async function todayRunWalkMix(userId: string, localDate: string): Promise<{ run: number; walk: number }> {
	const rows = await db.query<{
 ...
 */
async function crossTrainVariant(userId: string, localDate: string): Promise<CrossTrainVariant> {
...
async function findCompletingWorkout(userId: string, localDate: string, newWorkoutIds: string[]): Promise<string | null> {
...
async function getGoalMiles(userId: string): Promise<number> {
	const rows = await db.query<{ goal_miles: string }>(`SELECT goal_miles::text AS goal_miles FROM users WHERE user_id = $1`, [
		userId
	]);
 ...
 */
async function renderChallenge(userId: string, row: ChallengeRow): Promise<DailyChallenge> {
...
function shapeCrossTrain(row: ChallengeRow, variant: CrossTrainVariant): DailyChallenge {
	switch (variant) {
		case 'walk_today':
			return {
				key: row.challenge_key,
				title: 'Recovery Walk',
				description: "You've been logging miles — take a walking-only mile today to recover.",
		...
		case 'run_today':
			return {
				key: row.challenge_key,
				title: 'Lace ’Em Up',
				description: "It's been mostly walks lately — log your mile as a run today.",
				icon: 'figure.run',
