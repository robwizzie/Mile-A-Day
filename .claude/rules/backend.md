...
- A collab post has TWO owners, so nothing about it may be gated on friendship with `posts.user_id` alone — that 403'd hypes from the coauthor's friends after the client had played the animation. Post-context hypes/hyper-lists authorize on `visiblePostAuthors` (can you SEE it) and are always FILED against the primary author (`hype_count`/`is_hyped` count rows keyed to `posts.user_id`; a row against the coauthor is invisible AND skips dedupe) — the coauthor gets a push, not a row, and neither author may hype their own collab. Privacy is per author and asymmetric, since the post is the AUTHOR's content and the coauthor is a tag: author `'private'` → only the two of them see it; coauthor `'private'` → the tag and the reach it carries are withheld (`COAUTHOR_VISIBLE`/`COLLAB_REACH_SQL`) but the post stays with the author's circle. Both authors always see their own collab, so every `OWNER_NOT_PRIVATE_SQL` on a post needs the `p.user_id = $1 OR p.coauthor_user_id = $1` exemption (`AUTHOR_VISIBLE_TO_VIEWER`). Only `'private'` withholds — `'friends'` gates workout CONTENT, not identity.
