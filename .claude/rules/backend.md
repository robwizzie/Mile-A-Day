...
- Streak Assist availability (`getLiveAssistableBreak`) must NOT hide a rescue because the broken user holds a Double Down / Streak Save: the sweep returns "waiting" on an open DD window, so that hid the CTA for a whole day while the streak read broken everywhere. Report the token as `self_recovery` and let the giver's confirmation say so. Anything advertising a rescue (`getAssistableFriends`, `getFriendRescueStatus`) must re-run `giveStreakAssist`'s gap check (`stampedBreakDetail().bridged`) or the CTA promises a save the POST 409s. `prior_streak` is the run that ENDED at the miss — the number to show a user is `restored_streak` (prior + covered day + intact days since).
