...
- Goal-completion fresh window/photo prompt/composer must share `RunPostService.dailyMileWorkoutId()`; extra-mile prompts use the newest finished workout. Mixing ids causes prompt dedup/upsert bugs.
...
- Feed/post stats linked to a `workoutId` must come from that exact `HKWorkout` (distance/duration/pace/calories), not `todaysDistance`/day aggregates — extra-mile posts otherwise combine earlier walks/runs into one card.
...
- Any modal (toast, `.alert`, `.sheet`) rendered by a COVERED view never appears — true for a `.sheet` on top (see `FriendRequestsSheet`) and equally for a `.navigationDestination` push: ProfileView's Sign Out alert silently did nothing until you hit Back, because every button that triggered it lives on the pushed `ProfileSettingsView`. Whoever owns the button renders the confirmation; pass `@State` down as a `Binding` if the parent must own the action. Related: `FriendsListView` only exists once the Friends tab has been visited, so a `NotificationCenter` post aimed at it is dropped on cold launch — park intent on `DeepLinkRouter` and consume in BOTH `.task` and `.onReceive`.
- Data-entry sheets (`ManualWorkoutEntryView`, `EditWorkoutView`): `.decimalPad` has NO return key, so without a `ToolbarItemGroup(placement: .keyboard)` Done button the only escape is dragging the form — the same gesture that swipe-dismisses the sheet and discards the entry. Always pair the Done button with `.interactiveDismissDisabled(hasUnsavedInput)` + a discard confirmation on Cancel. Fixed-width `.wheel` pickers in a scrollable container leave dead gaps where a drag scrolls the form instead of spinning the wheel — give each wheel `.frame(maxWidth: .infinity)` and drop separator glyphs between them.
...
- `AppStateManager.isAuthenticated` is a UserDefaults bool, not proof of credentials — it's gated on `TokenStore.hasTokens` in memory only. Never persist that downgrade: tokens are `kSecAttrAccessibleAfterFirstUnlock`, so a pre-first-unlock background launch reads nil on a perfectly good session.
