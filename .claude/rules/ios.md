...
- `.offset` draws OUTSIDE layout bounds, so a stack whose look depends on it measures wrong and everything beside it centers off: the collab header's overlapping avatars occupied 44pt while measuring 40, floating the FRESH chip 2pt high. Build overlaps with `.padding` on the underlying view + a ZStack alignment instead. A collab post also carries two names where a solo one has one — chips sharing that row need `.fixedSize()` or they compress before the names do. `is_self` is the server's "you POSTED this", so it's false for an accepted coauthor: gate hype/report/block on an is-this-mine check that also counts `coauthor_user_id == currentUserId` (`PostCardView.isMine`), or a coauthor gets a Hype button on their own post that the server then 400s.
