...
- In-app tracking: GPS distance accrual must keep its noise gates (doppler-stationary skip, per-fix jitter floor, 12 m/s teleport cap, stale-fix rejection) — raw delta-summing inflates differently on every phone and de-syncs friends walking together. Outdoor finishes save `WorkoutLocationManager.reconciledFinalDistance` (walks prefer the parallel CMPedometer span on >10% disagreement), never raw `currentDistance`.
